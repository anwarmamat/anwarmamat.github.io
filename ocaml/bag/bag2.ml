...
(* Custom List based implementation of the BAG *)  
module MyListBag  (Elt:Stringable) : (BAG with type item = Elt.t) =
  struct
    type myList = Nil | Cons of Elt.t * myList  
    type item = Elt.t
    type t = myList
    let empty  = Nil
    let insert x bag = Cons (x, bag)
    let rec size bag = 
      match bag with 
      |Nil -> 0
      |Cons(_,t)-> 1 + size t
    ...
    let from_list lst b = 
        List.fold_left (fun bag item -> insert item bag) b lst
    
    let rec toString bag = 
      match bag with
        Nil->"[]"
       |Cons (h,t)-> "[" ^ (Elt.to_str h) ^ toString t ^"]" 
  end
 
(* create a IntBag  module *)  
module IntBag = MyListBag(IntItem);;
...
module PointBag = MyListBag(PointItem);;
